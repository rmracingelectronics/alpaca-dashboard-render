...
class LiveStore:
    ...
    def upsert_position(self, position: dict[str, Any], plan: dict[str, Any] | None = None) -> None:
        ...
        data = {
            ...
            "stop_price": self._float_or_none((plan or {}).get("stop_price")),
            "target_price": self._float_or_none((plan or {}).get("target_price")),
            "entry_reference_price": self._float_or_none((plan or {}).get("entry_reference_price")),
            "risk_budget": self._float_or_none((plan or {}).get("risk_budget")),
            "risk_per_share": self._float_or_none((plan or {}).get("risk_per_share")),
            "order_mode": (plan or {}).get("order_mode"),
            "extended_hours": 1 if bool((plan or {}).get("extended_hours", False)) else 0,
            "strategy_variant": (plan or {}).get("strategy_variant"),
            "strategy_code": (plan or {}).get("strategy_code"),
            "quality_gate": (plan or {}).get("quality_gate"),
        ...
        update_exprs.extend(
            [
                ...
                "entry_reference_price=COALESCE(excluded.entry_reference_price, entry_reference_price)",
                "risk_budget=COALESCE(excluded.risk_budget, risk_budget)",
                "risk_per_share=COALESCE(excluded.risk_per_share, risk_per_share)",
                "order_mode=COALESCE(excluded.order_mode, order_mode)",
                "extended_hours=COALESCE(excluded.extended_hours, extended_hours)",
                "strategy_variant=COALESCE(excluded.strategy_variant, strategy_variant)",
                "strategy_code=COALESCE(excluded.strategy_code, strategy_code)",
                "quality_gate=COALESCE(excluded.quality_gate, quality_gate)",
